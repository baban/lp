    ...
    public class CSharpGrammar : Grammar
    {
       ...
       */
        ...
        State S188 (Inadequate)
          Shift items:
            member_access_segments_opt -> member_access_segments_opt ｷmember_access_segment 
            member_access_segment -> ｷ. Identifier 
            member_access_segment -> ｷarray_indexer 
            array_indexer -> ｷ[ expression_list ] 
            member_access_segment -> ｷargument_list_par 
            argument_list_par -> ｷ( argument_list_opt ) 
            member_access_segment -> ｷtype_argument_list 
            type_argument_list -> ｷ_<_ type_ref_list > 
            _<_ -> ｷ< 
          Reduce items:
            member_access -> identifier_ext member_access_segments_opt ｷ [? , ) : ; } ] Identifier ++ -- || && | ^ & == != > <= >= << >> + - * / % = += -= *= /= %= &= |= ^= <<= >>= is as ?? <]
